"""OpenClipArt - adapted from SearXNG."""
...
logger = logging.getLogger("scoutlet.engines.openclipart")
...
def response(resp):
    ...
    for result in eval_xpath_list(dom, "//div[contains(@class, 'gallery')]/div[contains(@class, 'artwork')]"):
        results.append({
            ...
            "url": base_url + (extract_text(eval_xpath(result, "./a/@href")) or ""),
            "title": extract_text(eval_xpath(result, "./a/img/@alt")),
            "img_src": base_url + (extract_text(eval_xpath(result, "./a/img/@src")) or ""),
        })
