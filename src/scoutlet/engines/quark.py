"""Quark (Shenma) search engine - adapted from SearXNG.
...
logger = logging.getLogger("scoutlet.engines.quark")
...
def is_alibaba_captcha(html_text):
    return bool(re.search(CAPTCHA_PATTERN, html_text))


def init(_):
    if quark_category not in ('general', 'images'):
        raise SearchEngineAPIException(f"Unsupported category: {quark_category}")
...
def request(query, params):
    ...
    if time_range_dict.get(params.get('time_range')) and quark_category == 'general':
...
def parse_finance_shuidi(data):
    content = " | ".join(
        info for info in [
            data.get('establish_time'), data.get('company_status'), data.get('controled_type'),
            data.get('company_type'), data.get('capital'), data.get('address'), data.get('business_scope'),
        ] if info
    )
    return {
...
def parse_ss_doc(data):
    ...
    return {
        "title": html_to_text(
            data.get('titleProps', {}).get('content') or data.get('title')
        ),
        "url": (
