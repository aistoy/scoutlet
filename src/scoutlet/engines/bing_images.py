"""Bing Images search engine - adapted from SearXNG.
...
- Added logger = logging.getLogger("scoutlet.engines.bing_images")
...
logger = logging.getLogger("scoutlet.engines.bing_images")
...
def response(resp):
    ...
    for result in dom.xpath('//ul[contains(@class, "dgControl_list")]/li'):
        ...
        img_format = (
            " ".join(result.xpath('.//div[@class="imgpt"]/div/span/text()')).strip().split(" · ")
        )
        source = " ".join(
