"""Acfun (videos) - adapted from SearXNG."""
...
logger = logging.getLogger("scoutlet.engines.acfun")
...
def _extract_video_data(video_block):
    try:
        ...
        cover_xpath = video_block.xpath('.//div[contains(@class, "video__cover")]/a/img/@src')
        video_cover = extract_text(cover_xpath[0]) if cover_xpath else ""
    ...
    for match in matches:
        try:
            ...
            for video_block in video_blocks:
                video_info = _extract_video_data(video_block)
                if video_info and video_info.get("title") and video_info.get("url"):
